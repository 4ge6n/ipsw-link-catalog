(() => {
  ...
  const script = (queue, dest, jobs, existing, prune) => [
    ...
    'work="$(mktemp -d)"',
    "trap 'touch \"$work/monitor.stop\" 2>/dev/null || true; rm -rf \"$work\"' EXIT",
    'printf 0 > "$work/count"',
    "",
    ...
    `    rm -f "$other" && say "$(printf '     removed older build %s' "$other")"`,
    "  done",
    "}",
    "",
    "human() {",
    ...
    "}",
    "",
    "# The live block is redrawn in place under whatever has scrolled past, so",
    "# a message must clear it first or the two interleave.",
    ...
    '  [ "$drawn" -gt 0 ] && printf "\\033[%dA\\033[J" "$drawn" >&2',
    '  printf 0 > "$work/block"',
    "}",
    ...
    '      IFS="$(printf \'\\t\')" read -r active_name expected began from < "$progress_file" || continue',
    '      bytes="$(wc -c < "$active_name" 2>/dev/null | tr -d " ")"',
    '      awk -v name="$(model_key "$active_name")" -v bytes="${bytes:-0}" -v total="${expected:-0}" \\',
    ...
    `  say "$(printf '[%s/%s] %s (%s)' "$index" "$total" "$name" "$(human "$size")")"`,
    '  started=$SECONDS',
    '  if [ "$existing" = "redownload" ] && [ -f "$name" ]; then rm -f "$name"; fi',
    '  resume_from="$( [ -f "$name" ] && wc -c < "$name" | tr -d " " || echo 0 )"',
    '  printf "%s\\t%s\\t%s\\t%s\\n" "$name" "$size" "$(date +%s)" "$resume_from" > "$work/progress.$index"',
    ...
    `      say "$(printf '  !! SHA-1 MISMATCH for %s (expected %s, got %s)' "$name" "$expected_sha1" "$actual_sha1")"`,
    '      mv -f "$name" "$name.sha1-mismatch"',
    '      echo "$name (SHA-1 mismatch; saved as $name.sha1-mismatch)" >> "$work/failed"',
    '      return 1',
    '    fi',
    `    say "$(printf '     SHA-1 verified %s' "$name")"`,
    '  fi',
    '  [ "$prune" = "delete" ] && prune_old "$name"',
    '  done_count="$(record)"',
    '  percent=$((done_count * 100 / total))',
    `  say "$(printf '     done %s in %ss  —  %s of %s (%s%%)' "$name" "$((SECONDS - started))" "$done_count" "$total" "$percent")"`,
    "}",
    "",
