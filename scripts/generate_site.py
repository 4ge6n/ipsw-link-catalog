...
def generate(api: Path, output: Path):
    ...
    home=["<h1>Apple IPSW download links</h1><p>Direct Apple CDN links, organized by OS, release channel, version, and build. IPSW files are not hosted here.</p><ul>"]
    for os_key in OS_ORDER:
        ...
        for channel in ("release", "beta"):
            ...
            channel_page=[f"<p>{link('../', '← '+OS_NAMES[os_key])}</p><h1>{OS_NAMES[os_key]} {channel.title()}</h1><p>{link('latest/', 'Latest supported downloads')}</p><ul>"]
