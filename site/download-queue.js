(() => {
  ...
  const script = (queue, dest, jobs, existing, prune) => [
    ...
    "      IFS=\"$(printf '\\t')\" read -r active_name expected < \"$progress_file\"",
    "      case \"$expected\" in ''|0|*[!0-9]*) percent='?' ;; *)",
    "        bytes=$(wc -c < \"$active_name\" 2>/dev/null | tr -d ' '); percent=$((bytes * 100 / expected)); [ \"$percent\" -gt 100 ] && percent=100 ;; esac",
    "      active=\"${active}${active_name} ${percent}%;  \"",
    "    done",
    "    [ -n \"$active\" ] && printf '\\r\\033[K      active: %s' \"$active\" >&2",
    "    sleep 1",
    "  done",
    "}",
    "",
    ...
    "  printf '\\r\\033[K' >&2",
    `  printf '      finished %s in %ss — overall %s/%s (%s%%)\\n' "$name" "$((SECONDS - started))" "$done_count" "$total" "$percent"`,
    "}",
    "",
